...
import autoGeneratedTools from './autoGeneratedTools.json';
...
// ── 3. EXAM & GOVERNMENT PORTAL PRESETS ───────────────────────────────────────
const EXAM_PRESETS = [
...
const EXAM_TOOLS: Tool[] = EXAM_PRESETS.map((p) => ({
  ...
  slug: p.slug,
  name: p.name,
  category: p.slug.includes('signature') || p.slug.includes('declaration') ? 'signature' : 'photo',
  ...
  toolType: 'image-target-kb',
  badge: 'Exam Preset',
  keywords: [p.name.toLowerCase(), p.slug.replace(/-/g, ' '), 'official exam preset'],
  instructions: ['Select your photograph', `Engine formats to ${p.desc}`, 'Download application-ready file'],
  faq: [
    {
      question: 'Does this meet official government notification specs?',
...
// ── 4. IMAGE FORMAT CONVERTERS & MODIFIERS ───────────────────────────────────
...
// ── 6. CONSOLIDATED MASTER TOOLS REGISTRY (WITH AUTO-DEDUPLICATION) ───────────
const rawCombinedTools: Tool[] = [
  ...CORE_TOOLS,
  ...KB_TOOLS,
  ...EXAM_TOOLS,
  ...CONVERTER_TOOLS,
  ...MODIFIER_TOOLS,
  ...PDF_EXTRA_TOOLS,
  ...((Array.isArray(autoGeneratedTools) ? autoGeneratedTools : []) as Tool[])
];

const seenSlugs = new Set<string>();

export const TOOLS: Tool[] = rawCombinedTools.filter((tool) => {
  if (!tool || !tool.slug) return false;
  const normalized = tool.slug.toLowerCase().trim();
