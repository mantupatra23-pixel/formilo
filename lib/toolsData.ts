import rawGeneratedTools from './autoGeneratedTools.json';
...
// 2. Strict Deduplication Map Engine
function buildUniqueRegistry(): ToolItem[] {
  ...
  // Merge autoGeneratedTools.json without duplicates
  if (Array.isArray(rawGeneratedTools)) {
    for (const raw of rawGeneratedTools) {
      const slugKey = (raw.slug || raw.id || '').toLowerCase().trim();
      if (slugKey && !toolMap.has(slugKey)) {
        toolMap.set(slugKey, {
          id: raw.id || slugKey,
          slug: slugKey,
          title: raw.title || slugKey.replace(/-/g, ' ').toUpperCase(),
          description: raw.description || `Resize and format for online form strictly under ${raw.targetKB || 50} KB.`,
          category: (raw.category as any) || 'exam',
          badge: raw.badge || 'EXAM',
          ...
          dimensionText: raw.dimensionText || `${raw.width || 350} × ${raw.height || 450} px`,
          isFeatured: Boolean(raw.isFeatured),
          route: raw.route || `/exam/${slugKey}`
        });
      }
    }
  }
...
// 3. Exports for Pages & Components
