...
export default function ExamResizerTool({ preset }: { preset: PresetProps }) {
  ...
  // Smart Fit Mode: Signature ke liye 'contain' (no cut), Passport photo ke liye 'cover'
  const isSignature = preset.slug.includes('signature') || preset.slug.includes('sign');
  const [fitMode, setFitMode] = useState<'contain' | 'cover'>(isSignature ? 'contain' : 'contain');
  ...
  const handleImageUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        setImageElement(img);
        setFitMode('contain'); // Default 100% Full View bina kate
        setZoom(1.0);
        setRotation(0);
        setPanX(0);
        setPanY(0);
  ...
  const renderHDCanvas = useCallback(() => {
    ...
    // High Resolution 2x Scale for Crystal Clear Text & Face
    ...
    // Center Origin Translation & Rotation
    ctx.save();
    ...
    if (fitMode === 'contain') {
      // 100% Full Photo Fit (No Crop / No Cut-off)
    ...
    // Binary Search Quality for Maximum Sharpness under Target KB
    let minQ = 0.20;
    let maxQ = 0.98;
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-zinc-400 max-w-sm">
              HD bi-cubic resolution &bull; Target: {preset.minKB} KB – {preset.maxKB} KB
            </p>
          </div>
