...
import { Upload, Download, RefreshCw, MessageCircle, ShieldCheck, CheckCircle, Sliders } from 'lucide-react';
...
export default function ExamResizerTool({ preset }: { preset: PresetProps }) {
  ...
  const processImage = (imageSrc: string, targetQuality: number) => {
    ...
    img.onload = () => {
      ...
      // Iterative Compression under Max KB
      let q = targetQuality;
  ...
  const handleWhatsAppShare = () => {
    const text = `⚡ I just resized my ${preset.examName} ${preset.docType} strictly under ${preset.maxKB} KB for free on Formilo:\n${window.location.href}`;
    window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(text)}`, '_blank');
  };

  return (
    <div className="w-full bg-[#0c0d0e] border border-zinc-800 rounded-3xl p-6 md:p-8 shadow-2xl">
      <input
        type="file"
        ref={fileInputRef}
      ...
      {!selectedImage ? (
        <div
          ...
          className="border-2 border-dashed border-zinc-800 hover:border-emerald-500 rounded-2xl p-12 text-center cursor-pointer transition-all duration-200 bg-zinc-950/60 hover:bg-emerald-950/10 flex flex-col items-center justify-center gap-4"
        >
        ...
        <div className="space-y-6">
          {/* Dual Preview Box (Original vs Formatted) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center justify-center">
            ...
            <div className="space-y-2 text-center">
              ...
              <div className="relative aspect-[3.5/4.5] max-h-72 mx-auto rounded-2xl bg-black border border-zinc-800 flex items-center justify-center overflow-hidden p-2">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={selectedImage} alt="Original" className="max-h-full object-contain rounded-xl" />
              </div>
            </div>
            ...
            <div className="space-y-2 text-center">
              <span className="text-[11px] font-bold tracking-wider uppercase text-emerald-400 flex items-center justify-center gap-1">
                <CheckCircle className="w-3 h-3" /> Formatted ({outputSize} KB / {preset.dimensions.width}x{preset.dimensions.height}PX)
              </span>
              <div className="relative aspect-[3.5/4.5] max-h-72 mx-auto rounded-2xl bg-black border-2 border-emerald-500/80 shadow-lg shadow-emerald-500/10 flex items-center justify-center overflow-hidden p-2">
                {isProcessing ? (
                  <RefreshCw className="w-8 h-8 text-emerald-400 animate-spin" />
                ) : processedImage ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={processedImage} alt="Formatted Result" className="max-h-full object-contain rounded-xl" />
                ) : null}
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800 space-y-2">
            <div className="flex justify-between text-xs text-zinc-400 font-medium">
              <span className="flex items-center gap-1.5"><Sliders className="w-3.5 h-3.5 text-emerald-400" /> Fine-Tune File Size</span>
          ...
          {/* Action Buttons Matching Original UI */}
