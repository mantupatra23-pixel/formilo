...
export default function HomePage() {
  ...
  // Automatic Merged Matrix: Core + JSON Presets + Auto-Generated Action Tools
  const allTools = useMemo<UnifiedTool[]>(() => {
    ...
    // 2. Add Programmatic Auto-Generated Tools (From GitHub Actions)
    ...
    rawAutoList.forEach((item: any) => {
      ...
      if (!toolMap.has(cleanSlug)) {
        let cat: 'photo' | 'signature' | 'pdf' | 'converter' | 'exam' = 'exam';
        if (cleanSlug.includes('signature') || cleanSlug.includes('sign') || cleanSlug.includes('thumb')) {
          cat = 'signature';
        } else if (cleanSlug.includes('pdf')) {
          cat = 'pdf';
        } else if (cleanSlug.includes('photo') || cleanSlug.includes('postcard')) {
          cat = 'photo';
        }

        toolMap.set(cleanSlug, {
          id: item.id || cleanSlug.replace(/[^a-zA-Z0-9-]/g, '-'),
          slug: cleanSlug,
          name: item.name || item.title || 'Government Exam Resizer',
          description: item.description || item.desc || 'Free online document and photo resizer.',
          category: item.category || cat,
          badge: item.badge || item.board || 'EXAM',
          targetKB: item.targetKB || item.kb || 50,
          dimensions: item.dimensions || item.dim || '350 × 450 px',
          isPopular: false,
        });
      }
    });

    // 3. Track Base Exams to generate standard 4-variants matrix
    ...
    rawList.forEach((item: any) => {
      ...
      if (!baseSlug || seenExamBases.has(baseSlug)) return;
