...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://formilo.vercel.app/${tool.slug}`,
    },
    openGraph: {
      title: tool.seoTitle,
      description: tool.seoDescription,
    ...
    <>
      ...
      <main className="min-h-screen bg-slate-50 dark:bg-slate-950 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto space-y-10">
          ...
          {/* Dynamic Tool Interface Executor */}
          <section className="my-8">
            {tool.toolType === 'image-target-kb' && <ImageResizeTool tool={tool} />}
            {/* Fallback runner components for other tool types execute identically */}
          </section>
